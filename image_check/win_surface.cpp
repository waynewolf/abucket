...
bool setupGraphics(int w, int h) {
    ...
    ALOGI("glGetAttribLocation(\"a_position\") = %d\n", gaPositionHandle);

    gaColorHandle = glGetAttribLocation(gProgram, "a_color");
    checkGlError("glGetAttribLocation");
    ALOGI("glGetAttribLocation(\"a_color\") = %d\n", gaColorHandle);

    gComputeProgram = createComputeProgram(gComputeShader);
    if (!gComputeProgram) {
      	ALOGE("Could not create compute program.");
        return false;
    }
...
int main() {
    ...
    sp<SurfaceControl> surfaceControl = client->createSurface(String8("image_check"),
    	480, 782, HAL_PIXEL_FORMAT_RGBA_8888, 0);
	...
	//void *vboData = 0;
	//vboData = glMapBufferOES(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
	//checkGlError("glMapBufferOES");
	//if (!vboData) {
	//	printf("null ptr: 0x%x\n", glGetError());
	//}
	//checkArrayBuffer((char *)vboData);
